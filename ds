...
def get_all_comments(url):
    ...
    for i in range(page):  # 逐页抓取
        ...
        for item in json_dict['comments']:
            ite[u'comment'] = item['content'] # 评论内容
            ite[u'likedCount'] = item['likedCount'] # 点赞总数
            ite[u'comment_time'] = item['time'] # 评论时间(时间戳)
            ite[u'userID'] = item['user']['userId'] # 评论者id
            ite[u'nickname'] = item['user']['nickname'] # 昵称
            ite[u'avatarUrl'] = item['user']['avatarUrl'] # 头像地址
            # ite['comment_info'] = unicode(userID) + u" " + nickname + u" " + avatarUrl + u" " + unicode(comment_time) + u" " + unicode(likedCount) + u" " + comment + u"\n"
            all_comments_list.append(ite)
            # print ite
        # print("第%d页抓取完毕!" % (i+1))
    save_to_file()
...
# 将评论写入文本文件
def save_to_file():
    aa = json.dumps(all_comments_list)
...
if __name__ == "__main__":
    ...
    url = "http://music.163.com//weapi/v1/resource/comments/R_SO_4_254574?csrf_token=90e04572eb42b040167323ec2fcdd79f"
